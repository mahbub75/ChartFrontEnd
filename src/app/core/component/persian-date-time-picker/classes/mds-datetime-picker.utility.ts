...
export class MdsDatetimePickerUtility {
  ...
  static dateTimeToString(date: Date, format: string = ''): string {
    if (format === '' || format == null) {
      return `${this.zeroPad(date.getFullYear(), '0000')}/${this.zeroPad(date.getMonth() + 1, '00')}/${this.zeroPad(date.getDate(),
        '00')}   ${this.zeroPad(date.getHours(), '00')}:${this.zeroPad(date.getMinutes(), '00')}:${this.zeroPad(date.getSeconds(), '00')}`;
    }
